...
module.exports = function(host) {
  ...
  function hincr(key, field, increment) {
    increment = +increment || 1;
    client.hincrby(prefix + key, field, increment);
  }
  ...
  function getProjectList(cb) {
    ...
    var maxAge = 10 * 24 * 60 * 60 * 1000; // 10 days
    client.lrange(prefix + "projects", 0, -1, function(err, jobs) {
      ...
      if (!err && jobs) {
        ...
        var jobsSorted = _.sortBy(jobs, 'created');
        // Delete expired objects
        filteredJobs = jobsSorted.filter(function(job){
          var age = now - job.created;
          if(age > maxAge) {
            var val = JSON.stringify(job);
            client.lrem(prefix + "projects", 0, val);
            client.del(prefix + job.id);
            client.del(prefix + "jobInfo:" + job.id);
            return false;
          }
          return true;
        });
