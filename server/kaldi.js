...
var request = require('request'),
    ...
    redisHost = require("../settings/").redisHost,
    redis = require("redis"),
...
var kaldiBaseURL = "http://zgbwcsttapi04.labs.jupiter.bbc.co.uk/api/v0.2/stt",
    kaldiPoll;
...
function get(req, res) {
  ...
  q.defer(poll,job)
   ...
   .await(function(error,_,result){
    if(kaldiPoll.status == 'SUCCESS') {
      // Log processing time
      const duration = Math.round(req.query.duration);
      const time = Math.round((Date.now() - req.query.time)/1000);
      console.log("KALDI TIME TAKEN =", duration, time, kaldiPoll.status);
      redisClient.sadd(`audiogram:kaldi`, JSON.stringify([duration, time]));
    }
...
function stats(req, res){
  redisClient.smembers(`audiogram:kaldi`, (err, data) => {
    if (err) return res.json({err});
    let csv = 'sourceDuration,processingTime';
    data.forEach((line) => {
      let timings = JSON.parse(line);
      csv += '\n' + timings.join(',');
    });
    console.log(csv);
    res.write(csv);
